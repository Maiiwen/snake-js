function getAllIndexes(e, t) {
  for (var s = [], a = -1; -1 != (a = e.indexOf(t, a + 1)); ) s.push(a);
  return s;
}
function startGame(e) {
  (snakeClass = new Game(50, 5, e)),
    snakeClass.init(),
    document.body.addEventListener('keydown', function (e) {
      switch ((snakeClass.getStartTime(), e.keyCode)) {
        case 37:
          (snakeClass.movX = 0), (snakeClass.movY = -1);
          break;
        case 38:
          (snakeClass.movY = 0), (snakeClass.movX = -1);
          break;
        case 39:
          (snakeClass.movX = 0), (snakeClass.movY = 1);
          break;
...
let speed = 80,
  Game = class {
    ...
    constructTable() {
      let e = [];
      for (let t = 0; t < this.gamesize; t++) {
        e[t] = [];
        for (let s = 0; s < this.gamesize; s++) e[t][s] = 0;
      }
      return e;
    }
    createTable() {
      this.table = this.constructTable();
      for (let e = 0; e < this.gamesize; e++) {
        this.tableEl.insertRow();
        for (let t = 0; t < this.gamesize; t++)
          this.tableEl.rows[e].insertCell();
      }
    }
    updateTable() {
      for (let e = 0; e < this.gamesize; e++)
        for (let t = 0; t < this.gamesize; t++)
          (this.tableEl.rows[e].cells[t].innerHTML = ' '),
            0 !== this.table[e][t]
              ? this.tableEl.rows[e].cells[t].classList.add('crt')
              : this.tableEl.rows[e].cells[t].classList.remove('crt');
    }
    placeMice() {
    ...
    placeSnake() {
      let e = Math.round(this.gamesize / 2);
      (this.snake.headX = e), (this.snake.headY = e);
    }
    isEating() {
    ...
    moveSnake() {
      void 0 !== this.table[this.snake.headX + this.movX]
        ? (null !=
    ...
    getStartTime() {
      void 0 === this.timeStart &&
        ((this.timeStart = new Date()), console.log(this.timeStart));
    }
    logScore() {
      (document.querySelector('#addScoreForm').style.display = 'block'),
        (document.querySelector('#hiddenInput1').value = this.score);
      let e = (this.timeStop.getTime() - snakeClass.timeStart.getTime()) / 1e3;
      console.log(e),
        (document.querySelector('#hiddenInput2').value = e),
        (document.querySelector('#hiddenInput3').value = this.mode);
    }
  },
  playerName = document.querySelector('#playerName'),
  modeBtn = document.querySelector('#start'),
  addScoreButton = document.querySelector('#addScore');
addScoreButton.addEventListener('click', function () {
  ...
  playerName.addEventListener('input', function () {
    playerName.value.length < 50 && playerName.value.length > 2
